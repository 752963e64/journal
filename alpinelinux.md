...
- e2fsprogs is collection of tools to format your filesystems to suit Linux® & Co
...
EFI manager adds utilities but forces us to use FAT filesystem in middle common bootloader FS... I don't want that! so the system will boot as 32bit and then Linux® switch to 64bit.
...
Linux® doesn't need a C library to build but a C compiler, Linux® is built using strictly the assembler and the C abstraction. This means it relies entirely on the assembler and C language without any library to produce Linux®! The C language depend on assembler! The assembler is the CPU language idiosyncratic and damn powerfull... nothing on earth can compete with it... 😙

To match the late madness... **Things to be understood!** The Rust case! Rust is a language like C in his context, it uses the machine language to build its products. The main criteria of rust is memory management at machine level and language exotism.
...
Linus Torvalds is a prodige, Dr Richard Matthew Stallman is another one. Nowaday Linux® is the most engineered software in the world! It compiles all the main industries expertise that make the regulated free world spin literally.
...
With the Time advancing things evolves... Most contributions to Linux® is made from the industries (education, science, govs, media industries, devices maker) However there is also lot of individual contributing to the well being of Linux®. Linus Torvalds and Dr Richard Matthew Stallman set the basis from all of this and like a perpetual move It pursued these basis to become the most engineered software in the **WORLD**! Everybody can contribute to Linux®.
